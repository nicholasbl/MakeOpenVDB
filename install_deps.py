...
def run_subproc(s: Source, args, cwd=None):
    ...
    def append_env(varname, content):
        old_var = new_env[varname] if varname in new_env else ""
        if old_var:
            old_var = ":" + old_var
        new_env[varname] = content + old_var

    if (this_platform == "apple"):
        append_env("DYLD_LIBRARY_PATH", os.path.join(thirdparty_dir, "lib"))
